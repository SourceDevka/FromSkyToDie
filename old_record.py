...
class OSMO_SDR:   ## Soapy based SDR class
    ...
    def stream_threading(self, data_file_path='',log_file='',spectr_file=''):
        buff = numpy.array([0]*1024*32, numpy.complex64)
        # start streaming
        self.state='streaming'

        #--prepare IQ data stream file
        data_file_stream = None # data_fid
        if data_file_path != '':
            data_file_stream = open(data_file_path, "wb")
            if self.verbose :print  ("streaming to [{0}]".format(data_file_path))
        
        
        #prepare spectrum file
        spector_file_stream = None
        spectr_write_heading =True

        if spectr_file != '':
            spector_file_stream = open(spectr_file, "wt")
        ...
        while not self.stream_break:
            ...
            if data_file_stream is not None: buff.tofile(data_file_stream)
            ...
            if self.FFT_PERIOD > 0 :
                if time.time() - prev_shot_time > self.FFT_PERIOD: #fetch a FFT shot
                    if self.NO_FFT:
                        ...
                        #-- save spectrum
                        if spector_file_stream is not None: #dump spectr to file
